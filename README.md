...
群聊先按引用和连续发言形成初步候选，在参与门控允许且有实质文本时通过 `chat.understand` 理解语义话题、参与价值和回复对象，包括无引用的跨成员讨论。直接 @ 或引用 Agent 保持目标优先；静默模式、冷却和退避仍在模型分析前生效。分析结果随 Turn 冻结，统一用于回复计划、记忆查询及学习情境，重试复用原结果。

私聊中的短承接、指代和继续讨论会结合最近历史消解对象；没有历史依赖的输入直接生成回复。证据需求按真正缺少的信息判断，不再因“今天”“之前”等单词机械调用工具。当前请求及选中的前文优先占用上下文预算，画像、摘要、记忆和学习表达按需退让。行为反馈按对应回复划分证据范围，并区分行为效果与答案内容好评；弱延续最多更新 0.1 分，不能归因的好评不更新行为分数。

`chat.understand` 使用现有聊天端点和任务路由，可通过 `model.task_profiles` 单独指定模型或超时。真实模型模式会在上述情形增加一次分析调用；本地 Fake 模式仅重放确定性候选，不代表真实语义理解效果。
...
[任务循环熔断器](plugins/circuit_breaker/README.md) 默认自动启用：识别工具重复无进展、短周期循环和连续失败，并为整次工具任务设置超时；触发后立即结束当前执行，记录原因，保留已完成工具的真实审计。
